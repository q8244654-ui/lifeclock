...
export default function QuizPage() {
  ...
  function askQuestion(userNameOverride?: string) {
    setIsTyping(true)
    setCanAnswer(false)
  ...
  async function startPhase(initialUserName?: string, phaseIndexOverride?: number) {
  ...
  const handleAnswer = (option: (typeof currentQuestion.options)[0]) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setMessages(prev =>
          prev.map((msg, idx) =>
            idx === lastUserMessageIndex ? { ...msg, showReadReceipt: false } : msg
          )
        )
      }, 1500)
